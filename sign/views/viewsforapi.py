...
def add_event(request):
    ...
    try:
        Event.objects.create(event_id=eid, name=name, limit=limit, status=int(status), address=address,
                             start_time=start_time)
    except ValidationError:
        error = 'start_time format error,It must be in YYYY-MM-DD HH:MMM:SS format'
        return JsonResponse({'status': 10024, 'message': error})
    return JsonResponse({'status': 200, 'message': 'add event success'})


# 查询发布会接口
def get_evebt_list(request):
...
def user_sign(request):
    ...
    if not result:
        return JsonResponse({'status':10026,'message':'user did not paticipate in the conference'})
